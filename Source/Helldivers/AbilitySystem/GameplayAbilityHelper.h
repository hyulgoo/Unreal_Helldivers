...
public:
	...
	static void			                SendGameplayEventToTarget(const FGameplayTag EventTag, AActor*Instigator, AActor* TargetActor, const float Magnitude = 0.f);
	static void			                SendGameplayEventToTarget(const FGameplayTag EventTag, AActor*Instigator, AActor* TargetActor, const FGameplayEventData& Payload);
	...
	static void			                SendGameplayEventToSelf(const FGameplayTag EventTag, AActor* SourceActor, const FGameplayEventData& Payload);
    ...
    static void                         ExcuteGameplayCue(const FGameplayTag Tag, const FVector& Location, const FVector Normal, AActor* OwningActor);
    ...
    static void                         ExcuteGameplayCue(const FGameplayTag Tag, const FVector& Location, const FVector Normal, UAbilitySystemComponent* ASC);

    static void                         RemoveActiveEffectByGrantedTag(const FGameplayTag Tag, UAbilitySystemComponent* ASC);

    static const FGameplayTagContainer  GetAllChildTag(const FGameplayTag ParentTag);
};
